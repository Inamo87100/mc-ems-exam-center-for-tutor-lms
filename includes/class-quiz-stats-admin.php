...
class MCEMS_Quiz_Stats_Admin {
    ...
    public static function handle_recalc() {
        ...
        wp_safe_redirect(self::get_page_url(['updated' => 1]));
        exit;
    }
    ...
    protected static function get_filtered_stats($args, $do_count = false) {
        ...
        if ($args['min_error'] !== '' && is_numeric($args['min_error'])) {
            $where[] = 'error_percentage >= %f';
            $params[] = (float) $args['min_error'];
        }
        if ($args['max_error'] !== '' && is_numeric($args['max_error'])) {
            $where[] = 'error_percentage <= %f';
    ...
    public static function render_admin_page() {
        ...
        $course_id = isset($_GET['course_id']) ? (int) $_GET['course_id'] : ($courses[0]->ID ?? 0);
        $quiz_title = isset($_GET['quiz_title']) ? sanitize_text_field(wp_unslash($_GET['quiz_title'])) : '';
        $question_text = isset($_GET['question_text']) ? sanitize_text_field(wp_unslash($_GET['question_text'])) : '';
        $min_error = isset($_GET['min_error']) ? (string) wp_unslash($_GET['min_error']) : '';
        $max_error = isset($_GET['max_error']) ? (string) wp_unslash($_GET['max_error']) : '';
        ...
        <div class="wrap">
            ...
            <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" style="margin-bottom: 1em; display:inline-block;">
            ...
            <form method="get" action="<?php echo esc_url(admin_url('edit.php')); ?>" style="margin:16px 0;padding:10px;background:#f8f8fa;border-radius:8px;display:flex;gap:16px;flex-wrap:wrap;">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-bottom:1em;display:inline-block;">
                ...
                <input type="hidden" name="min_error" value="<?php echo esc_attr($min_error); ?>" />
                <input type="hidden" name="max_error" value="<?php echo esc_attr($max_error); ?>" />
        ...
        if (empty($results)) {
            ...
            foreach ($results as $row) {
                ...
                echo '<td style="text-align:right">' . (int) $row->total_answers . '</td>';
                echo '<td style="text-align:right">' . (int) $row->wrong_answers . '</td>';
