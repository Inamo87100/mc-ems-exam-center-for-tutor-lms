...
class NFEMS_Admin_Sessioni {
    ...
    public static function render(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nfems_action'])) {
            $action = sanitize_text_field(wp_unslash($_POST['nfems_action']));
            ...
            <div class="card" style="max-width: 1100px;">
                ...
                <form method="post" id="nfems-generate-form">
                    ...
                    <div id="nfems_gen_standard">
                        <table class="form-table">
                            <tr>
                                <th><label for="nfems_course_id"><?php echo esc_html__('Tutor LMS course', 'mc-ems'); ?></label></th>
                                <td>
                                    <?php if (!$course_pt): ?>
                            ...
                            <tr>
                                <th><label for="nfems_date_start"><?php echo esc_html__('Start date', 'mc-ems'); ?></label></th>
                                <td>
                                    <input
                                        type="date"
                                        id="nfems_date_start"
                            ...
                            <tr>
                                <th><label for="nfems_date_end"><?php echo esc_html__('End date', 'mc-ems'); ?></label></th>
                                <td>
                                    <input
                                        type="date"
                                        id="nfems_date_end"
                    ...
                    <div id="nfems_gen_special" style="display:none;">
                        <table class="form-table">
                            <tr>
                                <th><label for="nfems_special_course_id"><?php echo esc_html__('Tutor LMS course', 'mc-ems'); ?></label></th>
                                <td>
                                    <?php if (!$course_pt): ?>
                                    ...
                                    <?php elseif (!$courses): ?>
                                        <em><?php echo esc_html__('No published Tutor LMS course found.', 'mc-ems'); ?></em>
                                    <?php else: ?>
                                        <select id="nfems_special_course_id" name="special_course_id" disabled>
                                            <option value=""><?php echo esc_html__('— Select course —', 'mc-ems'); ?></option>
                                            <?php foreach ($courses as $cid => $title): ?>
                                                <option value="<?php echo (int) $cid; ?>"><?php echo esc_html($title); ?></option>
                                            <?php endforeach; ?>
                            ...
                            <tr>
                                <th><label for="nfems_special_date"><?php echo esc_html__('Date', 'mc-ems'); ?></label></th>
                                <td>
                                    <input
                                        type="date"
                                        id="nfems_special_date"
                                        name="special_date"
                                        value="<?php echo esc_attr($today); ?>"
                                        min="<?php echo esc_attr($today); ?>"
                                        disabled
                            ...
                            <tr>
                                <th><label for="nfems_special_time"><?php echo esc_html__('Time (single)', 'mc-ems'); ?></label></th>
                                <td>
                                    <input
                                        type="time"
                                        id="nfems_special_time"
                                        name="special_time"
                                        value=""
                                        disabled
                                    >
                    ...
                    <p class="submit">
                        <button type="submit" class="button button-primary" name="nfems_submit_generate" value="1">
        ...
        (function(){
            ...
            const specialCourse = document.getElementById('nfems_special_course_id');
            const specialDate   = document.getElementById('nfems_special_date');
            const specialTime   = document.getElementById('nfems_special_time');
            const specialEmail  = document.getElementById('nfems_special_user_email');

            const standardCourse = document.getElementById('nfems_course_id');
            const standardStart  = document.getElementById('nfems_date_start');
            const standardEnd    = document.getElementById('nfems_date_end');
            const standardTimes  = document.getElementById('nfems_times');
            const standardCap    = document.getElementById('nfems_capacity');
        ...
        (function(){
            const genForm = document.getElementById('nfems-generate-form');
            if (!genForm) return;
            ...
            genForm.addEventListener('submit', function(e){
                ...
                if (!isSpecial) {
                    ...
                    const sTime = document.getElementById('nfems_special_time');
                    const sUser = document.getElementById('nfems_special_user_email');
    ...
    private static function session_exists(string $date, string $time, int $course_id, bool $special_only = false): bool {
        ...
        if ($special_only) {
            $meta[] = ['key' => NFEMS_CPT_Sessioni_Esame::MK_IS_SPECIAL, 'value' => 1];
        }
